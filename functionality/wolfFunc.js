    ...
    wolfram.query(args, function (err, result) {
        if(args.includes("graph")) {
            if(result[1].subpods[1]){
                ...
                var wolfNum = result[1].subpods[0].image;
                callback(wolfNum)
            }
        }
        else if (args.includes("integrate") ||
            args.includes("differentiate")) {
            var wolfNum = result[0].subpods[0].value;
            callback(wolfNum)
        }
            ...
            args.includes("/") ||
            args.includes("^")) {
            var wolfNum = result[1].subpods[0].value;
            callback(wolfNum)
        }
        else callback("Please enter a valid argument (+,-,*,/,^,{integrate [function]},{differentiate [function]},{graph [function]})")
    });
};
